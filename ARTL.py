...
ticker = sys.argv[1]
tiker_live = sys.argv[1]

argument2 = 0
argument3 = 0

# Récupération des arguments passés depuis le script1.py
mots = sys.argv[1:]

# ----- initialisation de l'API key et ticker -----#

def supprimer_mottxt(ticker):
    # Connexion FTP
    ftp = FTP('ftp.cluster030.hosting.ovh.net')  # remplacer 'hostname' par le nom d'hôte du serveur
    ftp.login(user='abtrado', passwd='4a9dFrYgfrSB')  # remplacer 'username' et 'password' par vos identifiants FTP

    # Télécharger le fichier
    with open('mots.txt', 'wb') as fp:
        ftp.retrbinary('RETR mots.txt', fp.write)

    # Lire le fichier et supprimer le groupe de mots
    with open('mots.txt', 'r') as file:
        lines = file.readlines()

    word_to_remove = f"{ticker}"
    new_lines = []
    skip_count = 0
    ...
    # Écrire dans le fichier avec le groupe de mots supprimé
    ...
    with open('mots.txt', 'rb') as fp:
        ftp.storbinary('STOR mots.txt', fp)
